...
| Options | `calculateImpliedVolatility`, `cancelCalculateImpliedVolatility`, `calculateOptionPrice`, `cancelCalculateOptionPrice`, `exerciseOptions` | Implemented, but not fully promoted. Exact sv210 classic and sv211 protobuf calculation captures and public replay cover successful price and implied-volatility results; cancellation-before-first-result remains open. The guarded sv225 exercise campaign qualified a live ITM AAPL call, but warning 399 deferred its zero-fill seed order to the next options session and reconciliation proved no mutation. Positive exercise, lapse, override, and clearing settlement remain matrix work. |
