...
| Contracts/reference | `reqContractDetails`, `reqMatchingSymbols`, `reqSecDefOptParams`, `reqSmartComponents`, `reqMarketRule` | Implemented. Regular contract details preserve the complete classic v200 response, including the FUND tail. Exact sv205 protobuf requests are live-frozen for stock, bond, fund, option, and issuer selectors. Bond issuer lookup and the distinct message-18 bond details shape are live-attested; empty coupon/maturity/rating fields remain unset rather than inferred. |
