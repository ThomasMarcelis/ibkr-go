...
| OPT-002 | Option exercise/lapse | `Options().Exercise`, official `exerciseOptions` | `api_option_exercise_aapl`, `api_option_exercise_not_itm_aapl.txt`, `api_option_exercise_server_reject_aapl.txt` | promoted | Both live outcomes frozen 2026-06-11: a barely-ITM call drew 322 "not in-the-money" (silently dropped — no route for the exercise req id), and a deep-ITM call was accepted (10349 TIF-preset session event, the only public trace; the Gateway materializes a pseudo-order under the exercise req id whose frames match no route) then rejected by the paper clearing server with 322 and a 202 the engine also drops. Lapse and override variants plus a true clearing settlement remain open. |
