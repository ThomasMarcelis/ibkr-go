...
| SESS-002 | Session observation and current time | `Client.CurrentTime`, `Client.CurrentTimeMillis`, official `reqCurrentTime`, `reqCurrentTimeInMillis`, `currentTime`, `currentTimeInMillis`, session events | `current_time`, `current_time_millis`, `grounded_bootstrap.txt`, `error_farm_status_codes.txt`, `current_time_live.txt`, `current_time_millis.txt` | promoted | Exact sv206 raw replays freeze both seconds (`c4ad2ec73d6d2a92`) and millisecond (`3070c6c9296d0eb2`) request/reply paths. The exact bootstrap replay proves farm-status callbacks leave the session snapshot unchanged. Unavailable-current-time behavior has no live evidence and stays a watch item. |
...
| ID | Capability | Public API / Official Surface | Current Scenarios / Replay | Status | Required Matrix Variants |
|----|------------|-------------------------------|----------------------------|--------|--------------------------|
| ERR-001 | API errors and farm status | official `error` callback, system status codes | `error_api_error_oneshot.txt`, `realtime_bars_api_error.txt`, `error_empty_results.txt`, `error_market_data_warning.txt`, `error_farm_status_codes.txt` | promoted | Request-scoped one-shot errors, exact subscription-closing code-420 refusal, session-scoped statuses, warning-only behavior, and advanced order reject JSON. The fabricated quote code-354 replay was removed. |
