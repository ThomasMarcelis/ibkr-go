...
| MD1-003 | Quote streams and generic ticks | `MarketData().SubscribeQuotes`, official tick callbacks and `rerouteMktDataReq` | `quote_stream_aapl`, `quote_stream_genericticks`, `quote_with_generic_ticks`, `quote_stream_multi_asset`, `api_market_data_completeness_aapl`, `api_generic_tick_matrix_aapl`, `api_tick_news_aapl_probe`, `tick_news_aapl_sv201_live.txt`, `market_data_sv206_live.txt`, `api_duplicate_quote_subscriptions_aapl`, `api_duplicate_quote_subscriptions_aapl.txt` | candidate | The public `QuoteUpdate` union preserves every price/size callback with its numeric tick type, price-attribute mask, and optional companion size, including ticks that have no normalized `Quote` field. Numeric generic, string, request-parameter, option-computation, and contract-specific news events remain distinct. Exact sv206 protobuf requests/callbacks and presence-aware precision are replay-promoted from captures `eea31798e7e59830f` and `989563f9c4cad108e3`; classic CFD reroutes are live-attested by `7475841869bc53ce` and transparently replace the active request. Existing sv200/sv201 generic and TickNews captures remain valid. TickEFP, additional generic families, and entitled live-price streams remain gaps; duplicate same-contract subscriptions are replay-promoted from capture `84f1e78a18616e0f`. |
...
| SCAN-002 | Scanner subscriptions | `Scanner().SubscribeResults`, official subscription/cancel callbacks | `scanner_subscription`, `api_scanner_subscription`, `scanner_subscription_live.txt` | promoted | Public-API server-version-200 capture `c84c81b3ee772bcc` freezes the complete 25-field request, ten ranked `scannerData` rows, and clean cancellation; the curated replay preserves the exact result frame. Generic filter values and `tag=value;` serialization are grounded in official source. Older capture `740b5dfb138df2a4` retains real permission-denied codes 490 and 365. No-result scans and non-default filter captures remain evidence gaps. |
