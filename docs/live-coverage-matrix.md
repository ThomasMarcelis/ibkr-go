...
| SESS-005 | Reconnect and interruption | reconnect policy, transport loss, API 1100/1101/1102 | `api_reconnect_active_order_aapl`, `api_reconnect_active_order_aapl.txt`, `api_order_handle_reconnect_cancel_aapl.txt`, `reconnect_oneshot_interrupted.txt`, `quote_stream_reconnect.txt`, direct engine fault tests | candidate | Active GTC order reconnect and original order-handle Gap/RecoveryRequired/cancel are replay-promoted. Transport resubscription uses exact sv206 quote frames. Data-lost/data-maintained connectivity behavior is frozen by direct engine fault injection rather than synthetic wire transcripts; a raw 1100/1101/1102 capture would strengthen evidence but no longer blocks transcript provenance. Active account streams and broader real-time-bar coverage remain open. |
| SESS-006 | Lifecycle edge contracts | subscription close, context cancel, singleton limits, slow consumer, concurrent one-shots | lifecycle fixtures, including `lifecycle_concurrent_oneshots.txt` | promoted | The concurrent-one-shot replay combines exact sv200 AAPL and EURUSD contract-details frames, rebinds request IDs, and deliberately reorders the captured responses to freeze demultiplexing. Add live-derived singleton/account-limit variants where possible. |
| SESS-007 | Negotiated protocol train | version gates 200..225, official API 10.48 migration table | codec exact vectors, public routing tests, `protocol-audit-sv203.md` through `protocol-audit-sv208-225.md` | promoted | Every protobuf migration through 213 and each implemented semantic gate through 225 fails closed at its lower boundary. The sv214 outbound UTC suffix remains unresolved, and sv221 configuration mutation is intentionally outside the public API. Positive entitlement evidence remains owned by the affected capability row rather than weakening version negotiation. |
...
| ACCT-002 | Account updates and portfolio | `Accounts().Updates`, `Accounts().SubscribeUpdates`, official `reqAccountUpdates`, `updateAccountValue`, `updatePortfolio`, `updateAccountTime`, `accountDownloadEnd` | `account_updates`, `account_updates.txt` | promoted | The finite public snapshot is live-attested at exact 206, 207, and current 225. Capture `20260714T200703Z-account_updates` returned 194 values, 18 portfolio rows, 19 timestamps, and the end marker (events `43f1637f84252a7`, replay frames `bb6908aed896bf3`). The promoted minimal exact-sv225 replay freezes one arm of every callback family plus unsubscribe (`56fca257974a67c7`). During-market-order deltas and the one-account timing limitation remain broader streaming targets. |
...
| ID | Capability | Public API / Official Surface | Current Scenarios / Replay | Status | Required Matrix Variants |
|----|------------|-------------------------------|----------------------------|--------|--------------------------|
| MD1-001 | Market data type control | `MarketData().SetType`, official `reqMarketDataType`, `marketDataType` | `set_type_live`, `set_type_frozen`, `set_type_delayed`, `set_type_delayed_frozen`, `set_type_switch_while_streaming`, `api_market_data_completeness_aapl`, `api_market_data_type_cycle.txt`, `quote_snapshot.txt`, `set_type_switch_while_streaming.txt` | promoted | Bare SetType(1/2/3/4) accepted silently (2026-04-15 capture `f692fc168a53da9d`); the 2026-06-11 readonly-live capture proves the stream-tied behavior: marketDataType(3) arrives before delayed ticks, mid-stream SetType(Live) is accepted without a type-1 acknowledgement, and request-scoped 10167 is delivered on the owning quote stream (`e244cae7f5eb7d57`). Exact snapshot replay freezes delayed selection and its type-3 response (`a854ac41c4e0073f`); the public streaming scenario requires typed delayed-type plus price/size evidence before a fenced switch and cancellation (`6040ff478db4d277`). The inconclusive raw invalid-type scenario was deleted; public validation already rejects values outside 1..4 before the wire. Real-time (type 1) success pushes still need a market-hours entitled stream. |
| MD1-002 | Quote snapshots | `MarketData().Quote`, `MarketData().RegulatorySnapshot`, official `reqMktData`, `cancelMktData`, `tickSnapshotEnd` | `quote_snapshot_aapl`, `api_market_data_completeness_aapl`, `quote_snapshot.txt` | candidate | Exact sv206 replay freezes delayed-data selection, the resolved AAPL ordinary snapshot request, all twelve typed quote fields, and the snapshot-end boundary (`a854ac41c4e0073f`). Regulatory snapshots have distinct classic/protobuf encoder-law coverage and use the same completion marker. Four explicitly authorized 2026-07-11 probes reached live systems: the read-only Gateway returned generic internal-server errors at sv207 and sv200, while paper sv207 returned typed restriction code 10213 for AAPL. On 2026-07-15, exactly one newly authorized readonly-live request at sv225 again returned definitive code 10213; the verified private capture has event hash `42e466e6af9ecb65` and was not retried. A positive quote result remains required before promotion. OPT/FUT/CASH snapshots, entitlement error, and no data remain broader targets. |
| MD1-003 | Quote streams and generic ticks | `MarketData().SubscribeQuotes`, official tick callbacks and `rerouteMktDataReq` | `quote_stream_aapl`, `quote_stream_genericticks`, `quote_stream_multi_asset`, `quote_odd_lot_aapl`, `api_market_data_completeness_aapl`, `api_generic_tick_matrix_aapl`, `api_tick_news_aapl_probe`, `tick_news_aapl_live.txt`, `market_data_sv206_live.txt`, `api_duplicate_quote_subscriptions_aapl`, `api_duplicate_quote_subscriptions_aapl.txt` | candidate | The public `QuoteUpdate` union preserves every price/size callback with its numeric tick type, price-attribute mask, and optional companion size. Ordinary and 233/236 generic streams are public-API live-verified. Exact sv206 protobuf requests/callbacks and presence-aware precision are replay-promoted; exact-sv206 classic and sv225 protobuf market-data CFD reroutes are live-attested. TickEFP, delta-neutral validation, and odd-lot tick IDs 105-110 are typed and source-law frozen. The exact-sv225 generic-787 probe produced 19 ordinary delayed updates and the real code-10167 notice but no odd-lot values (`70500b2228dc29e8`), matching the official SDK entitlement boundary (`85d0dba58ba9d80c`). Positive EFP, delta-neutral, odd-lot, and entitled live-price streams remain evidence gaps, not implementation gaps. |
| MD1-004 | Tick callback edge shapes | official `tickPrice`, `tickSize`, `tickString`, `tickGeneric`, `tickEFP`, `tickOptionComputation`, `tickNews`, `tickReqParams`, `deltaNeutralValidation` | `quote_stream_genericticks`, `tick_efp_probe`, `quote_odd_lot_aapl`, `api_generic_tick_matrix_aapl`, `api_tick_news_aapl_probe`, `tick_news_aapl_live.txt`, `api_option_campaign_aapl`, `api_option_calculations_aapl`, `option_calculations_aapl_live.txt` | candidate | TickPrice attributes and companion size, mapped and unmapped TickPrice/TickSize IDs, an omitted TickReqParams minimum tick, TickGeneric, TickString, TickNews, and option-computation success/sentinel shapes are live-attested and publicly delivered. TickEFP and delta-neutral validation now have typed public variants and exact official-layout tests; positive callbacks require an entitled active product/BAG. Odd-lot projection has an exact negative-entitlement live boundary at sv225. |
...
| HIST-005 | Historical ticks | `History().Ticks`, official historical midpoint/bidask/last callbacks | `historical_ticks_aapl_trades`, `historical_ticks_aapl_bidask`, `historical_ticks_aapl_midpoint`, `historical_ticks_aapl_timezone_start`, historical tick transcripts | promoted | Exact sv206 replay freezes the resolved AAPL end-bounded request and complete 367-trade terminal response (`ecc715f0a1aea00c`). Exact current sv207 BID_ASK and MIDPOINT requests terminate with typed code-10187 permission refusals (`17485ca8784b646da76c9e4fac56dcaec37644e7dbe53695ddf6f494a434509d`, `23eaeccef26263af212651bc235cfa1a73739173a0c2eeec836512f83a98ac13`); their former success rows were invented and removed. Start-only requests, explicit-zone formatting, successful BID_ASK/MIDPOINT callbacks, and multi-callback `done=false` accumulation remain unattested. |
...
| ID | Capability | Public API / Official Surface | Current Scenarios / Replay | Status | Required Matrix Variants |
|----|------------|-------------------------------|----------------------------|--------|--------------------------|
| ORD-001 | Basic order placement | `Orders().Place`, official `placeOrder`, `openOrder`, `orderStatus` | existing basic/order-type campaigns plus `order_lifecycle_sv203_live.txt` | promoted | Classic families remain frozen by the 2026-06-11 campaigns. Exact sv203 now mirrors the official encoder's explicit `Contract.conId=0` behavior; fresh guarded paper capture `8efd714c3885da23` proves place/open/status and targeted cancel. Its later global cancel was live-flushed and returned code 161 because the sole order was already terminal. Classic place carries SecurityID/full BAG legs/delta-neutral but rejects IncludeExpired and IssuerID; sv203 uses the complete shared Contract. Deferred `IncludeOvernight` coverage has two focused scenarios. At exact sv200, place a safe nonmarketable one-share SMART DAY LMT with the flag true, observe an explicit true classic echo, replace true to false using the current empty encoding, observe explicit false, cancel to terminal state, and fence with CurrentTime; if empty does not clear true, canonical `"0"` becomes a separate evidence-backed change. At exact sv203, stage the same safe order with protobuf field 135 true and `Transmit=false`, observe the staged state and no execution during a bounded window, replace with `Transmit=true`, observe accepted/open state, cancel to terminal state, and fence with CurrentTime. Preserve negotiated version, capture ID, sanitized raw frames and hashes, cancellation, and clean-account evidence for both runs. |
...
| ORD-005 | Open orders | `Orders().Open`, `Orders().SubscribeOpen`, `OpenOrdersSubscription.Refresh`, official open/all/auto-open-order scopes and `orderBound` | existing open/reconnect/cross-client captures and replays; `TestLiveManualTWSOrderBound` capture harness | candidate | Own/all/client-0/cross-client/reconnect/refresh paths are promoted. Classic and protobuf OpenOrder project the same complete `OrderDetails`; dual delivery deep-clones every mutable Contract, combo, tag, condition, and decimal payload. `orderBound` decoding and client-0 routing are official-schema implemented, and the opt-in harness validates positive binding IDs plus clean auto-binding teardown, but raw paper-TWS evidence remains a disclosed v2.0.0 follow-up gap. |
| ORD-006 | Completed orders | `Orders().Completed`, `Orders().StreamCompleted`, official `reqCompletedOrders`, `completedOrder`, `completedOrdersEnd` | `api_completed_orders_variants_aapl`, `api_completed_orders_variants_aapl.txt`, `completed_orders`, `completed_orders_empty.txt`, `completed_orders_cancelled_system_live.txt`, `completed_orders_sv204_live.txt` | promoted | The slice API delegates to a bounded finite stream. Exact sv206 evidence freezes both outcomes: the paper snapshot ends normally with an empty result (`9f7bdc11f8012a5e`), while the read-only role's unkeyed `-'S'` code-321 refusal is a typed terminal result followed by a current-time fence (`ff9808ba8f3f33f2`). At exact sv207, official Python SDK 10.48.01 and candidate `30754b08c7a9` both sent the same present-true protobuf request and received no result, end marker, or request error before 15 seconds (`98ec10ce208e65bf`). Server 225 repeated that silence through ibkr-go and official C++ SDK 10.46.01. The library therefore returns caller-context expiry as unresolved and never fabricates an empty snapshot. The classic codec parses every source-defined field sequentially, rejects unparsed drift, and projects the complete typed v200 result. Exact sv204 is official-SDK/live-attested for absent-false and present-true requests plus cancelled and filled protobuf results; the filled frame preserves commission-and-fees amount/currency, and optional order/client/parent identities preserve explicit zero. Nondefault combo, scale-extension, hedge, active delta-neutral, PEG BENCH, condition, and FA completed-order branches remain live-attestation gaps. |
| ORD-007 | Executions and commissions/fees | `Orders().Executions`, `Orders().SubscribeExecutions`, `Orders().SubscribeExecutionEvents`, official `reqExecutions`, `execDetails`, `commissionAndFeesReport` | `executions_snapshot`, `executions.txt`, `executions_correlated.txt`, `executions_overlapping.txt`, `api_order_fill_aapl` | promoted | Raw sv200 captures freeze all 33 execution fields, all 8 commission-and-fees wire fields, the execution-end marker, and fee reports on both sides of that marker. The one-shot preserves fees seen through the boundary; the query subscription stays open for later/revised reports. The passive client-wide observer sends no request and exposes every execution and fee callback before query correlation or order-handle deduplication. Account/symbol filters and the required unset last-days/date tail are live-attested. Client, time, secType, exchange, side, finite last-days, and specific-date filters remain live targets. Liquidation, EV/model, pending-revision, submitter, and meaningful bond yield/redemption values also remain unattested beyond defaults. |
| ORD-008 | Order handle lifecycle | `OrderHandle.Events`, `OrderHandle.Lifecycle`, `OrderHandle.Done`, `OrderHandle.Wait`, `OrderHandle.Close` | `api_reconnect_active_order_aapl`, `api_order_handle_reconnect_cancel_aapl.txt`, `api_transmit_false_then_transmit_aapl`, order replay and live tests | promoted | explicit caller-owned close after terminal status, late execution/fee delivery, detach without cancel, active-order reconnect/open-order recovery, and slow-consumer failure |
| ORD-009 | End-to-end trading campaign | account, orders, executions, completed orders, PnL, positions | `api_algorithmic_campaign_aapl`, `api_scale_in_campaign_aapl`, `api_scale_in_campaign_aapl.txt`, `api_stress_rapid_fire_aapl`, `api_stress_rapid_fire_aapl.txt`, `api_pairs_trading_aapl_msft`, `api_pairs_trading_aapl_msft.txt`, `api_dollar_cost_averaging_aapl`, `api_dollar_cost_averaging_aapl.txt`, `api_stop_loss_management_aapl`, `api_stop_loss_management_aapl.txt` | candidate | The public campaigns own split buys/sells, concurrent observers, cleanup, stop management (`a563cafd26e366be`), exact sv207 rapid-fire global cancel (`d66113abb8382887`), scale-in/protective stop (`63db2db7cba21b68`), pairs (`0dc806f7bb0868e8`), and repeated buys (`296bdf662eb84e30`). Final account/position/PnL/open-order reconciliation and later execution-query tails remain targets. Aggressive paper sizing defaults to 500-share campaign clips. |
...
| AORD-007 | Delta-neutral extensions | official delta-neutral order/contract fields and `deltaNeutralValidation` | `tick_efp_probe`; exact-200 rejected OPT request; source-schema tests | candidate | Public DeltaNeutral requires BAG. The captured OPT+delta-neutral quote was rejected with code 320 and is negative evidence only. The classic validation callback is decoded into a typed `QuoteUpdateDeltaNeutralValidation`; successful BAG quote/order/open/completed behavior and a positive validation callback remain live gaps. |
...
| AORD-009 | Pegged and adjusted order families | official PEG BENCH, PEG BEST, PEG MID, adjusted stop/trailing fields | `api_order_type_matrix_aapl`, `api_tif_attribute_matrix_aapl`; official source-law vectors | candidate | PEG BENCH placement validates and encodes all five reference fields plus auction reference prices; classic/protobuf OpenOrder preserve them. The matrix replay stops before its legacy tail-shifted PEG MID/BEST requests, which remain provenance-only. Existing adjusted fields remain strict. Correctly framed live accepted/rejected captures are still required for each specialized branch. |
...
| OPT-002 | Option exercise/lapse | `Options().Exercise`, official `exerciseOptions` | `api_option_exercise_aapl`, `api_option_exercise_not_itm_aapl.txt`, `api_option_exercise_server_reject_aapl.txt` | promoted | Both live outcomes frozen 2026-06-11: a barely-ITM call drew 322 "not in-the-money", and a deep-ITM call produced the 10349 TIF-preset warning plus a working pseudo-order before paper clearing returned 322 and 202. `ExerciseHandle` correlates warnings, request errors, and the pseudo-order lifecycle; none is treated as final settlement proof. Lapse and override variants plus a true clearing settlement remain open. |
...
| `tick_news_aapl_live.txt` | MD1-003, MD1-004 |
...
- `Options().Exercise`: add a positive exercise/lapse completion scenario;
  current paper evidence covers fill correlation, acknowledgement, refusal,
