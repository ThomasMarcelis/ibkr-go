...
| ID | Capability | Public API / Official Surface | Current Scenarios / Replay | Status | Required Matrix Variants |
|----|------------|-------------------------------|----------------------------|--------|--------------------------|
| ERR-001 | API errors and farm status | official `error` callback, system status codes | `contract_details_not_found.txt`, `realtime_bars_api_error.txt`, `error_empty_results.txt`, `error_market_data_warning.txt`, `error_farm_status_codes.txt` | promoted | Exact request-scoped one-shot code-200 and subscription-closing code-420 refusals, session-scoped statuses, warning-only behavior, and advanced order reject JSON. Duplicate symbolic one-shot and fabricated quote code-354 replays were removed. |
