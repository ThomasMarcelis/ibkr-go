...
| ORD-006 | Completed orders | `Orders().Completed`, official `reqCompletedOrders`, `completedOrder`, `completedOrdersEnd` | `api_completed_orders_variants_aapl`, `api_completed_orders_variants_aapl.txt`, `completed_orders`, `completed_orders.txt` | promoted | `api_completed_orders_variants_aapl` was recaptured on 2026-04-15 (`6ac84daaf4084436`) after fixing the live TRAIL LIMIT completed-order path; apiOnly=false and apiOnly=true both reached `completedOrdersEnd`. The codec now parses every source-defined classic field sequentially and rejects unparsed drift; the public result still needs the full projection. |
