...
| ACCT-002 | Account updates and portfolio | `Accounts().Updates`, `Accounts().SubscribeUpdates`, official `reqAccountUpdates`, `updateAccountValue`, `updatePortfolio`, `updateAccountTime`, `accountDownloadEnd` | `account_updates`, `account_updates.txt` | promoted | The catalog scenario now uses the finite public snapshot and exact sv206 evidence freezes 194 account values, 18 portfolio rows, the end marker, unsubscribe, and current-time fence (`67610b05ba57b095`). Every projected update is checked as an exact one-arm union. During-market-order deltas and the one-account timing limitation remain broader streaming targets. |
...
| TWS-002 | Display groups | `TWS().DisplayGroups`, `TWS().SubscribeDisplayGroup`, `DisplayGroupHandle.Update`, official display group calls | `display_groups`, `display_group_subscribe`, `display_groups.txt`, `display_group_subscribe.txt` | candidate | Both catalog scenarios now use the public API. Exact sv206 capture `bb3a476faf05a1d3` queried the real group list, subscribed to returned group 1, received typed initial value `none`, unsubscribed, waited, and fenced. Because the group was empty, the state-preserving update path correctly did not send an invalid `none` token. A clean live update of a nonempty group plus invalid group/update cases remain gaps; the ungrounded symbolic subscription fixture is not promoted evidence. |
