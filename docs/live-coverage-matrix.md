...
| SESS-003 | ID allocation and managed-account refresh | `Orders().RefreshOrderID`, `Client.ManagedAccounts`, official `reqIds`, `reqManagedAccts`, `nextValidId`, `managedAccounts` | `req_ids`, `managed_accounts_refresh`, `req_ids.txt`, `req_ids_read_only.txt`, `managed_accounts_refresh.txt`, bootstrap fixtures | promoted | `Orders().RefreshOrderID` is grounded both ways: the paper Gateway answers NEXT_VALID_ID (`req_ids.txt`), while the read-only live Gateway rejects with req_id=-1/code 321 (`req_ids_read_only.txt`, capture hash `00b11cbce4cefc31`). `Client.ManagedAccounts` is independently grounded by official-SDK and public-API exact-206 captures; the public capture (`dc7a53d00e621d95`) proves the second callback is routed to the caller and updates the session snapshot. Repeated allocation after orders remains a target variant. |
