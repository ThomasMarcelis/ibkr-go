...
readonly baseline="$root/testdata/api/v2.0.0-rc.2.api"
readonly approved_breaks="$root/testdata/api/approved-breaks-after-v2.0.0-rc.2.txt"
current="$(mktemp)"
...
if [[ -n "$changes" ]]; then
	printf 'incompatible public API changes since v2.0.0-rc.2:\n%s\n' "$changes" >&2
	exit 1
fi
