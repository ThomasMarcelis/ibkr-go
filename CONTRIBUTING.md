...
`ibkr-go` is a Go client for the Interactive Brokers TWS and IB Gateway socket protocol. The v1 target is a **read-only production core**. Order writes, scanners, news, Flex, and the Client Portal Web API are explicitly out of v1 scope. See [`docs/roadmap.md`](docs/roadmap.md) for the full charter.
