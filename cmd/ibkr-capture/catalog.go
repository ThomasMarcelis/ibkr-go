...
var scenarioMetadataByName = map[string]scenarioMetadata{
	...
	"api_option_exercise_aapl":              meta("options", []string{"Orders().Place", "Options().Exercise"}, []int{3, 5, 21}, "paper_marketable_order", []string{"paper_trading", "market_hours", "option_permissions"}, []string{"option fill then real exercise acknowledgement or no-position error"}, 1, "candidate", batchTrading),
	"api_hedge_order_aapl":                  meta("orders", []string{"Orders().Place"}, []int{3, 4, 5}, "paper_order", []string{"paper_trading"}, []string{"hedge child accept or real rejection per hedge type"}, 1, "candidate", batchTrading),
