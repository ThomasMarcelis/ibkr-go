...
var scenarioMetadataByName = map[string]scenarioMetadata{
	...
	"api_fundamental_reports_aapl":           apiMeta("contracts", []string{"Contracts().FundamentalData"}, []int{52, 53, 51, 4}, "entitlement_probe", []string{"fundamental_data_subscription_or_error"}, []string{"every FundamentalReportType returns XML or real Gateway entitlement/error response"}, 1, "candidate", []string{"STK"}, batchNewV2, batchReplayAll),
...
func (e scenarioCatalogEntry) inBatch(batch string) bool {
	...
	case batchExhaustivePermissionProbes:
		return e.RiskClass == "entitlement_probe" ||
			...
			hasString(e.Requirements, "fundamental_data_subscription_or_error") ||
