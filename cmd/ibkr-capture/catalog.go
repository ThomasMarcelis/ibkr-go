...
var scenarioMetadataByName = map[string]scenarioMetadata{
	...
	"api_wsh_variants_aapl":                  apiMeta("wsh", []string{"WSH().MetaData", "WSH().EventData"}, []int{100, 101, 102, 103, 104, 105, 4}, "entitlement_probe", []string{"wsh_subscription_or_error"}, []string{"WSH metadata, conid, portfolio, watchlist, and competitor variants return data or real entitlement errors"}, 1, "candidate", []string{"STK"}, batchNewV2, batchReplayAll),
	...
	"api_pairs_trading_aapl_msft":            apiMeta("orders", []string{"MarketData().Quote", "Orders().Place", "Orders().Executions"}, []int{1, 2, 3, 5, 11, 57, 58, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"paired AAPL buy and MSFT sell are accepted, filled, rejected, or flattened with real Gateway response"}, 1, "candidate", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
	"api_dollar_cost_averaging_aapl":         apiMeta("orders", []string{"Orders().Place", "Orders().Executions"}, []int{3, 5, 11, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"repeated AAPL buys over intervals followed by flattening and execution reconciliation"}, 1, "candidate", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
	...
	"api_bracket_trailing_stop_aapl":         apiMeta("orders", []string{"Orders().Place", "Orders().CancelAll", "Orders().Executions"}, []int{3, 5, 11, 58, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"bracket parent with take-profit and trailing-stop child is accepted, cancelled, and flattened with real Gateway response"}, 1, "candidate", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
	...
	"api_duplicate_quote_subscriptions_aapl": apiMeta("market_data", []string{"MarketData().SubscribeQuotes"}, []int{1, 2, 58}, "entitlement_probe", []string{"market_data_or_delayed_data"}, []string{"two same-contract quote subscriptions start independently or return real singleton/entitlement errors"}, 1, "candidate", []string{"STK"}, batchNewV2, batchReadOnly, batchExhaustiveReadOnly, batchReplayAll),
