...
var scenarioMetadataByName = map[string]scenarioMetadata{
	...
	"set_type_live":                         meta("market_data", []string{"MarketData().SetType"}, []int{59, 58}, "read_only", nil, []string{"marketDataType=1 push or real entitlement error"}, 1, "promoted", batchNewV2, batchReadOnly),
	"set_type_frozen":                       meta("market_data", []string{"MarketData().SetType"}, []int{59, 58}, "read_only", nil, []string{"marketDataType=2 push"}, 1, "promoted", batchNewV2, batchReadOnly),
	"set_type_delayed":                      meta("market_data", []string{"MarketData().SetType"}, []int{59, 58}, "read_only", nil, []string{"marketDataType=3 push"}, 1, "promoted", batchNewV2, batchReadOnly),
	"set_type_delayed_frozen":               meta("market_data", []string{"MarketData().SetType"}, []int{59, 58}, "read_only", nil, []string{"marketDataType=4 push"}, 1, "promoted", batchNewV2, batchReadOnly),
	"set_type_invalid":                      meta("market_data", []string{"MarketData().SetType"}, []int{59, 4}, "read_only", nil, []string{"real IBKR invalid data type error"}, 1, "promoted", batchNewV2, batchReadOnly),
	"set_type_switch_while_streaming":       meta("market_data", []string{"MarketData().SetType", "MarketData().SubscribeQuotes"}, []int{59, 1, 2, 58}, "read_only", []string{"market_data_or_delayed_data"}, []string{"setType push observed mid-stream"}, 1, "promoted", batchNewV2, batchReadOnly),
	...
	"tick_efp_probe":                        meta("market_data", []string{"official reqMktData EFP BAG"}, []int{1, 2, 4, 58, 59, 81}, "entitlement_probe", []string{"live_market_data", "active_single_stock_future", "matching_stock"}, []string{"TickEFP callback or real contract, entitlement, or no-data result"}, 1, "candidate", batchReadOnly),
	...
	"historical_bars_keepup":                meta("history", []string{"History().SubscribeBars"}, []int{20, 25, 17, 90, 108}, "read_only", []string{"historical_data"}, []string{"keep-up-to-date historical bars"}, 1, "promoted", batchReadOnly),
	...
	"scanner_subscription":                  meta("scanner", []string{"Scanner().SubscribeResults"}, []int{22, 23, 20}, "read_only", nil, []string{"scanner rows or real subscription error"}, 1, "candidate", batchReadOnly),
	...
	"market_rule":                           meta("contracts", []string{"Contracts().MarketRule"}, []int{91, 93}, "read_only", nil, []string{"price increment ladder"}, 1, "promoted", batchReadOnly),
	"place_order_lmt_buy_aapl":              meta("orders", []string{"Orders().Place", "OrderHandle.Cancel"}, []int{3, 4, 5}, "paper_order", []string{"paper_trading"}, []string{"far-from-market order accepted then cancelled"}, 1, "promoted", batchNewV2, batchTrading),
	"place_order_mkt_buy_aapl":              meta("orders", []string{"Orders().Place"}, []int{3, 5, 11, 59}, "paper_marketable_order", []string{"paper_trading", "market_hours"}, []string{"market buy fill or real market-state response"}, 1, "promoted", batchNewV2, batchTrading),
	"place_order_mkt_sell_aapl":             meta("orders", []string{"Orders().Place"}, []int{3, 5, 11, 59}, "paper_marketable_order", []string{"paper_trading", "market_hours", "position_or_short_permission"}, []string{"market sell fill or real rejection"}, 1, "promoted", batchNewV2, batchTrading),
	...
	"api_fa_replace_non_fa":                 apiMeta("advisors", []string{"Advisors().ReplaceConfig"}, []int{19}, "paper_order", []string{"paper_trading"}, []string{"real non-FA account response to FA group replacement"}, 1, "promoted", []string{"STK"}, batchTrading),
	"api_option_exercise_aapl":              meta("options", []string{"Orders().Place", "Options().Exercise"}, []int{3, 5, 21}, "paper_marketable_order", []string{"paper_trading", "market_hours", "option_permissions"}, []string{"option fill then real exercise acknowledgement or no-position error"}, 1, "promoted", batchTrading),
	"api_hedge_order_aapl":                  meta("orders", []string{"Orders().Place"}, []int{3, 4, 5}, "paper_order", []string{"paper_trading"}, []string{"hedge child accept or real rejection per hedge type"}, 1, "promoted", batchTrading),
	...
	"qualify_contract_ambiguous":            meta("contracts", []string{"Contracts().Qualify", "Contracts().Details"}, []int{9, 10, 52, 4}, "read_only", nil, []string{"ambiguous contract results or real ambiguity error"}, 1, "promoted", batchNewV2, batchReadOnly),
	...
	"api_order_rest_cancel_aapl":             apiMeta("orders", []string{"MarketData().SetType", "MarketData().Quote", "Orders().Place", "OrderHandle.Cancel"}, []int{1, 2, 3, 4, 5, 57, 58}, "paper_order", []string{"paper_trading"}, []string{"far LMT rest/cancel path"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingBasic, batchTradingAll, batchReplayDefault, batchReplayAll),
	"api_order_relative_cancel_aapl":         apiMeta("orders", []string{"MarketData().SetType", "MarketData().Quote", "Orders().Place", "OrderHandle.Cancel"}, []int{1, 2, 3, 4, 5, 57, 58}, "paper_order", []string{"paper_trading"}, []string{"REL rest/cancel behavior isolated because Gateway can reconnect during relative order validation"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	"api_order_trailing_cancel_aapl":         apiMeta("orders", []string{"MarketData().SetType", "MarketData().Quote", "Orders().Place", "OrderHandle.Cancel"}, []int{1, 2, 3, 4, 5, 57, 58}, "paper_order", []string{"paper_trading"}, []string{"TRAIL and TRAIL LIMIT behavior isolated because Gateway can reconnect during trailing validation"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	"api_order_stop_cancel_aapl":             apiMeta("orders", []string{"MarketData().SetType", "MarketData().Quote", "Orders().Place", "OrderHandle.Cancel"}, []int{1, 2, 3, 4, 5, 57, 58}, "paper_order", []string{"paper_trading"}, []string{"STP and STP LMT rest/cancel behavior isolated because Gateway can reconnect during stop validation"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	...
	"api_tif_attribute_matrix_aapl":          apiMeta("orders", []string{"Orders().Place", "OrderHandle.Cancel", "Orders().Executions"}, []int{3, 4, 5, 7, 11, 55}, "paper_order", []string{"paper_trading"}, []string{"GTC/GTD/GoodAfterTime/AON/MinQty/TrailingPercent/PercentOffset/Scale/Adjusted/ManualOrderTime/AdvancedErrorOverride accepted or rejected with real Gateway response"}, 1, "promoted", []string{"STK"}, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	"api_security_type_probe_matrix":         apiMeta("contracts", []string{"Contracts().Details"}, []int{9, 10, 52, 4}, "entitlement_probe", []string{"security_type_permissions_or_real_error"}, []string{"contract details or real rejection for STK/OPT/FUT/FOP/CASH/BOND/CFD/WAR/IND/CRYPTO/FUND/BILL/CMDTY/CONTFUT"}, 1, "candidate", []string{"STK", "OPT", "FUT", "FOP", "CASH", "BOND", "CFD", "WAR", "IND", "CRYPTO", "FUND", "BILL", "CMDTY", "CONTFUT"}, batchNewV2, batchReplayAll),
	"api_market_data_completeness_aapl":      apiMeta("market_data", []string{"MarketData().SetType", "MarketData().Quote", "MarketData().SubscribeRealTimeBars", "MarketData().SubscribeTickByTick"}, []int{1, 2, 45, 46, 50, 51, 57, 58, 59, 97, 98, 99}, "entitlement_probe", []string{"market_data_or_delayed_data"}, []string{"market data type pushes, generic ticks, real-time TRADES/BID_ASK/MIDPOINT, and tick-by-tick variants or entitlement errors"}, 1, "candidate", []string{"STK"}, batchNewV2, batchReplayAll),
	"api_generic_tick_matrix_aapl":           apiMeta("market_data", []string{"MarketData().SetType", "MarketData().SubscribeQuotes"}, []int{1, 2, 45, 46, 58, 59, 81}, "entitlement_probe", []string{"market_data_or_delayed_data"}, []string{"delayed AAPL stream preserves observed mark-price tick 37, shortable ticks 46/89, volume-rate tick 56, delayed timestamp tick 88, and omitted minimum-tick parameters"}, 1, "promoted", []string{"STK"}, batchNewV2, batchReadOnly, batchReplayAll),
	"api_tick_news_aapl_probe":               apiMeta("news", []string{"MarketData().SetType", "MarketData().SubscribeQuotes"}, []int{1, 2, 4, 58, 59, 81, 84}, "entitlement_probe", []string{"api_news_subscription"}, []string{"contract-specific BRFG TickNews or a real entitlement/no-new-headline result"}, 1, "promoted", []string{"STK"}, batchNewV2, batchReadOnly, batchReplayAll),
	"api_scanner_subscription":               apiMeta("scanner", []string{"Scanner().SubscribeResults"}, []int{22, 23, 20, 4}, "entitlement_probe", []string{"scanner_permissions_or_real_error"}, []string{"complete public scanner request returns ranked rows or the real Gateway subscription error"}, 1, "promoted", []string{"STK"}, batchNewV2, batchReadOnly, batchReplayAll),
	"api_historical_matrix_aapl":             apiMeta("history", []string{"History().Bars"}, []int{20, 17, 4}, "read_only", []string{"historical_data"}, []string{"all planned historical bar-size probes and whatToShow variants return data or real Gateway errors"}, 1, "candidate", []string{"STK"}, batchNewV2, batchReplayAll),
	"api_news_article_aapl":                  apiMeta("news", []string{"News().Historical", "News().Article"}, []int{84, 83, 86, 87, 80, 4}, "entitlement_probe", []string{"news_or_historical_news"}, []string{"article ID sourced from historical news is requested through reqNewsArticle or real entitlement/no-result is frozen"}, 1, "candidate", []string{"STK"}, batchNewV2, batchReplayAll),
	"api_wsh_variants_aapl":                  apiMeta("wsh", []string{"WSH().MetaData", "WSH().EventData"}, []int{100, 102, 4}, "entitlement_probe", []string{"wsh_subscription_or_error"}, []string{"WSH metadata plus conid, portfolio, watchlist, competitor, and date-window event-data variants return real code 10276 entitlement errors"}, 1, "promoted", []string{"STK"}, batchNewV2, batchReplayAll),
	"api_algo_variants_aapl":                 apiMeta("orders", []string{"Orders().Place", "OrderHandle.Cancel", "Orders().Executions"}, []int{3, 4, 5, 7, 11, 55}, "paper_order", []string{"paper_trading", "algo_permissions_or_real_error"}, []string{"Adaptive, TWAP, VWAP, ArrivalPx, DarkIce, AccumDist, Inline, Close, PctVol, BalanceImpactRisk, MinImpact, and Jefferies variants accepted, rejected, or cancelled with real Gateway response"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	"api_pairs_trading_aapl_msft":            apiMeta("orders", []string{"Orders().Place"}, []int{3, 5, 11, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"paired AAPL/MSFT market entries and per-symbol flatten fills; source execution-query tail timed out"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
	"api_dollar_cost_averaging_aapl":         apiMeta("orders", []string{"Orders().Place"}, []int{3, 5, 11, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"three staged AAPL market buys plus aggregate flatten fill; source execution-query tail timed out"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
	"api_stop_loss_management_aapl":          apiMeta("orders", []string{"Orders().Place", "OrderHandle.Modify", "OrderHandle.Cancel", "Orders().Executions"}, []int{3, 4, 5, 11, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"market entry, protective stop placement, stop modification, cancellation, flatten, and execution reconciliation"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
	"api_bracket_trailing_stop_aapl":         apiMeta("orders", []string{"MarketData().SetType", "MarketData().Quote", "Orders().Place", "Orders().CancelAll", "Orders().Executions"}, []int{1, 2, 3, 4, 7, 11, 55, 57, 58, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"live scenario probes quote, places market-parent bracket with TRAIL child, and receives code 328; promoted replay freezes request/rejection before execution-query and cleanup tail"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
	...
	"api_option_calculations_aapl":           apiMeta("options", []string{"Contracts().SecDefOptParams", "Contracts().Details", "MarketData().Quote", "Options().Price", "Options().ImpliedVolatility"}, []int{1, 2, 4, 10, 21, 52, 54, 55, 56, 57, 75, 76}, "read_only", []string{"market_hours", "option_permissions"}, []string{"live-qualified option price and implied-volatility result or real entitlement error"}, 1, "candidate", []string{"OPT"}, batchNewV2, batchReadOnly, batchReplayAll),
	...
	"api_completed_orders_variants_aapl":     apiMeta("orders", []string{"Orders().Place", "Orders().Completed", "Orders().Executions"}, []int{3, 5, 7, 11, 55, 59, 99, 101, 102}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"fresh paper fill followed by completed-orders apiOnly=false and apiOnly=true queries"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchExhaustiveMarketHours, batchReplayAll),
	"api_transmit_false_then_transmit_aapl":  apiMeta("orders", []string{"Orders().Place", "OrderHandle.Modify", "OrderHandle.Cancel"}, []int{3, 4, 5}, "paper_order", []string{"paper_trading"}, []string{"Transmit=false staged order is modified to transmit, then cancelled or rejected by the real Gateway"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchExhaustiveTrading, batchReplayAll),
	"api_duplicate_quote_subscriptions_aapl": apiMeta("market_data", []string{"MarketData().SetType", "MarketData().SubscribeQuotes"}, []int{1, 2, 58, 59}, "entitlement_probe", []string{"market_data_or_delayed_data"}, []string{"SetType(Delayed), then two same-contract quote subscriptions start independently and both receive delayed bid/ask ticks"}, 1, "promoted", []string{"STK"}, batchNewV2, batchReadOnly, batchExhaustiveReadOnly, batchReplayAll),
	"api_reconnect_active_order_aapl":        apiMeta("orders", []string{"Orders().Place", "Orders().Open", "Orders().Cancel"}, []int{3, 4, 5, 53}, "paper_order", []string{"paper_trading", "multi_leg_recorder"}, []string{"resting GTC order survives client reconnect and is visible/cancellable after reconnect"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchExhaustiveTrading, batchReplayAll),
	"api_client_id0_order_observation_aapl":  apiMeta("orders", []string{"Orders().Place", "Orders().Open", "Orders().Cancel"}, []int{3, 4, 5, 16, 53}, "paper_order", []string{"paper_trading", "client_id_0", "multi_leg_recorder"}, []string{"client ID 0 observes and cancels another client's live resting order"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchExhaustiveTrading, batchReplayAll),
	"api_cross_client_cancel_aapl":           apiMeta("orders", []string{"Orders().Place", "Orders().Open", "Orders().Cancel"}, []int{3, 4, 5, 16, 53}, "paper_order", []string{"paper_trading", "multi_client", "multi_leg_recorder"}, []string{"one client places a resting order and a second client observes/cancels it or returns the real Gateway rejection"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchExhaustiveTrading, batchReplayAll),
	"api_forex_lifecycle_eurusd":             apiMeta("orders", []string{"MarketData().SetType", "MarketData().Quote", "Orders().Place", "OrderHandle.Modify", "OrderHandle.Cancel"}, []int{1, 2, 3, 4, 5, 57, 58}, "paper_order", []string{"paper_trading", "forex_hours"}, []string{"EUR.USD far LMT reaches Inactive with real paper-account leverage rejection"}, 1, "promoted", []string{"CASH"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	"api_whatif_margin_aapl":                 apiMeta("orders", []string{"Orders().Place"}, []int{3, 4, 5}, "paper_order", []string{"paper_trading"}, []string{"WhatIf margin/commission preview or real Gateway parser/permission response without execution"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	"api_stress_rapid_fire_aapl":             apiMeta("orders", []string{"Orders().Place", "Orders().CancelAll"}, []int{3, 4, 5, 58}, "paper_order", []string{"paper_trading"}, []string{"10 rapid-fire far LMT orders plus global cancel"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingAdvanced, batchTradingAll, batchReplayAll),
	"api_scale_in_campaign_aapl":             apiMeta("orders", []string{"Orders().Place"}, []int{3, 5, 11, 59}, "paper_trigger", []string{"paper_trading", "market_hours"}, []string{"scale-in 2x MKT buy plus protective stop-loss PreSubmitted trigger; source capture tail timed out during cancel/flatten/execution query"}, 1, "promoted", []string{"STK"}, batchNewV2, batchTrading, batchTradingCampaigns, batchTradingAll, batchReplayAll),
...
func writeScenarioRole(w io.Writer, scenarioNames []string) error {
	...
	for _, name := range scenarioNames {
		...
		md, ok := scenarioMetadataByName[scenarioName]
		if !ok {
			return fmt.Errorf("unknown scenario %q", scenarioName)
		}
		if scenarioCaptureRole(md) == captureRolePaperDev {
