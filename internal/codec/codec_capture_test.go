...
func TestCaptureDecode_ManagedAccounts(t *testing.T) {
	...
	payload := []byte("15\x001\x00U10597365\x00")
	...
	if len(m.Accounts) != 1 || m.Accounts[0] != "U10597365" {
		t.Errorf("Accounts = %v, want [U10597365]", m.Accounts)
	}
}
...
func TestCaptureDecode_AccountSummaryValue(t *testing.T) {
	...
	payload := []byte("63\x001\x001001\x00U10597365\x00BuyingPower\x00266701.40\x00EUR\x00")
	msgs, err := DecodeBatch(payload)
	if err != nil {
		t.Fatalf("DecodeBatch: %v", err)
	}
	...
	if m.Account != "U10597365" {
		t.Errorf("Account = %q, want U10597365", m.Account)
	}
	if m.Tag != "BuyingPower" {
		t.Errorf("Tag = %q, want BuyingPower", m.Tag)
	}
	if m.Value != "266701.40" {
		t.Errorf("Value = %q, want 266701.40", m.Value)
	}
	if m.Currency != "EUR" {
...
func TestCaptureDecode_Position(t *testing.T) {
	...
	payload := []byte("61\x003\x00U10597365\x003691937\x00AMZN\x00STK\x00\x000.0\x00\x00\x00NASDAQ\x00USD\x00AMZN\x00NMS\x0015\x00199.66666665\x00")
	msgs, err := DecodeBatch(payload)
	if err != nil {
		t.Fatalf("DecodeBatch: %v", err)
	}
	...
	if m.AvgCost != "199.66666665" {
		t.Errorf("AvgCost = %q, want 199.66666665", m.AvgCost)
	}
}
