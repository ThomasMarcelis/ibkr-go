...
func TestDecodeScannerParametersProto225ExactLiveFrame(t *testing.T) {
	t.Parallel()

	// Capture 20260824T202844Z-scanner_parameters, events.jsonl SHA-256
	// 02ca289379189356eacedc56576be6d863e4a2b46feb8c28386c36ac07948ba7.
	message, err := Decode(225, readCapturedGzip(t, "testdata/scanner_parameters_sv225.gz"))
	if err != nil {
		t.Fatal(err)
	}
	parameters, ok := message.(ScannerParameters)
	if !ok {
		t.Fatalf("Decode() type = %T", message)
	}
	if len(parameters.XML) != 1_801_494 || !bytes.HasPrefix([]byte(parameters.XML), []byte("<?xml version=")) {
		t.Fatalf("ScannerParameters XML length = %d", len(parameters.XML))
	}
}
